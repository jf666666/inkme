...
class API {
  ...
  private func awaitRequest(_ api: APITargetType) async throws -> Data? {
    let url = api.baseURL.appendingPathComponent(api.path)
    var urlComponents = URLComponents(url: url, resolvingAgainstBaseURL: true)!
    ...
    let (receiveData, response) =  try await URLSession.shared.data(for: request)
    return receiveData
  }
}
    ...
    req(api)
      .mapError { error -> APIError in
        if case APIError.authorizationError = error {
          if type(of: api) is Splatoon3API.Type {
